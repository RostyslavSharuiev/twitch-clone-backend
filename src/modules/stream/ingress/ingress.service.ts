...
export class IngressService {
  ...
  public async create(user: User, ingressType: IngressInput) {
    ...
    if (!ingress || !ingress.url || !ingress.streamKey) {
      throw new BadRequestException('Ingress stream creating failed');
    }
  ...
  private async resetIngresses(user: User) {
    const ingressList = await this.livekitService.ingress.listIngress({
      roomName: user.id,
    });

    const roomList = await this.livekitService.room.listRooms([user.id]);
